...
`webapp/evidence.py` дополняет результат сведениями `validation` и `explanations`. Проверка контракта опирается на исходный план, публичную историю пилотов и детали оценщика: сверяются допустимые ресурсы, размеры пилотов, число и идентичность кампаний, расходы и контакты. Отсутствующее подтверждение имеет статус `not_checked`; это не успех. Отрицательная прибыль сама по себе не делает корректный контракт ошибочным. Worker не создаёт успешный экспорт, если обязательное подтверждение отсутствует или проверка провалена.

Время для проверки более строгого лимита 5 минут измеряется вокруг самого вызова `Agent.act`. В `agent_runtime_seconds` не включаются подготовка среды и работа оценщика. Общая длительность прогона сохраняется отдельно в `runtime_seconds`, а серверный таймаут процесса решает другую задачу — защиту очереди от зависания.

Объяснение кампании связывается с пилотами по целевому тарифу, каналу и фильтрам. Предпочитается точный сегмент; перенос с подходящего более широкого сегмента обозначается явно. Суммарный размер повторных пилотов называется тестовыми контактами, поскольку уникальность клиентов неизвестна. При отсутствии сопоставимого пилота показан резервный выбор без подтверждения эффекта. Альтернативы включаются только для той же пилотной ячейки и изменения одного решения — тарифа либо канала. Средняя оценка, стандартная ошибка и прогноз не выдаются за подтверждённый оценщиком эффект или доказательство глобального оптимума.

Доступны одиночный эксперимент, серия по последовательным seed и запуск тестов. В HTML/JSON отдельно показаны прогнозы агента и фактическая оценка локальной mock-среды. Экспорт `campaign_plan.csv` относится к выбранному эксперименту; официальный `submission.csv` формируется только из seed 42 при настройках агента по умолчанию. Корневые артефакты сдачи не изменяются. Ответ API содержит отдельное `submission` для стандартного seed 42; в серии оно может относиться не к выбранному лучшему эксперименту. Старые записи без новых полей остаются читаемыми и не получают успешную отметку проверки задним числом.

Существующий обзор показывает четыре шага: готовность данных, настройка запуска, реальные пилоты и проверка/скачивание. Кнопка стандартных настроек меняет форму без автоматического старта. Причина ошибки данных и ограничения задания видны около запуска; во время работы показаны последние три пилотных наблюдения.
...
| `webapp/evidence.py` | Публичные подтверждения контракта и объяснения по пилотам; модуль приложения, не часть алгоритма сдаваемого агента |
...
Набор содержит **60 тестов**: 16 тестов ядра, 24 теста сервера и данных, 5 тестов контракта worker и 15 новых тестов подтверждений результата, объяснений и связи с экспортом. Проверяются влияние наблюдений на решения, ресурсы, повторный `act`, перестановка строк, отрицательные пилоты, ошибки наблюдений, размеры аудитории, первый доступный пилот при ограниченных ресурсах и остановка поиска по дедлайну. Серверные проверки охватывают API, целостность всех CSV, границы доступа к файлам, отмену, повреждённую историю и ошибки записи состояния. Проверки worker отклоняют неправильный сырой план, потерю кампании при оценке и сбой событий прогресса до успешного экспорта, в том числе когда штатный оценщик перехватывает исключение. Новые регрессии различают соблюдение контракта и прибыль, исключают успешную проверку при пропущенных данных, подмену кампании при совпавшем числе записей и несогласованный учёт пилотов. Проверяются точное/широкое соответствие сегментов, альтернативы, отдельный seed файла сдачи и измерение времени именно `Agent.act`. `scripts/validate_web.py` выполняет настоящие HTTP-запуски и проверяет выгрузки, публичные подтверждения и объяснения, включая воспроизводимость официального CSV.

`scripts/validate.py` проверяет неизменность исходного оценщика, штатные прогоны, лимиты и повторную генерацию submission. Проверка повторена 23 сентября 2026 года: метрики seed 42 и серии 0–9 сохранились, файл сдачи воспроизводится. Финальная HTTP-проверка `scripts/validate_web.py` также пройдена: одиночный запуск, серия, экспорты и все 60 тестов, без ошибок, сбоев и пропусков. Метрики и результаты сохранены в `artifacts/validation.json` и `artifacts/web_validation.json`; подробности — в [requirements-audit.md](requirements-audit.md). Локальные результаты не предсказывают результат на скрытых эффектах.

Повторная проверка синтаксиса Python (`compileall`) пройдена. Для JavaScript доступна команда `node --check web/app.js`. Линтер и отдельная проверка типов не настроены и не установлены, поэтому не выполнялись. Сборки frontend нет: браузер получает исходные HTML, CSS и JavaScript.

Дополнительное исследование неизменённого агента выявило 19/20 положительных результатов на seed 10–29, включая −64 890,98 на seed 20. Два общих экспериментальных изменения — позднее подтверждение и более строгий порог 2SE — не приняты из-за ухудшения результатов на сопоставимых завершённых seed. Стратегия и её параметры не изменены; методика, фактические серии и ограничения сохранены в [strategy_review.json](../artifacts/strategy_review.json).
