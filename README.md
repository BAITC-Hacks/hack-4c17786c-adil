...
- **Адаптивные пилоты:** выбор сегментов, тарифов и каналов, обновление оценки эффекта по наблюдениям, повторные проверки и учёт неопределённости.
- **Планирование с ограничениями:** контроль бюджета, контактов, числа пилотов и финальных кампаний; проверка допустимости фильтров и тарифов.
...
- **Подготовка и проверка:** восстановление данных из архива, обзор аудитории, запуск тестов из интерфейса или терминала.
...
1. **Подготовка данных.** Приложение восстанавливает CSV из локального пакета участника. Пользователь видит состояние файлов и состав аудитории в разделе «Данные и проверки».
...
5. **Оценка и результат.** Локальный оценщик рассчитывает эффект и расходы. В интерфейсе появляются показатели, кампании, пилоты, объяснения, допущения и файлы для скачивания.
...
| [webapp/worker.py](webapp/worker.py) | Выполнение экспериментов и тестов, события прогресса, подготовка выгрузок |
...
Каждый запуск получает новое состояние агента и собственный каталог результатов. Задачи исполняются последовательно; интерфейс опрашивает API и остаётся доступным во время вычислений. После аварийного закрытия незавершённые задачи помечаются прерванными.

Агент использует публичные данные и методы среды. Доступ к внутренней модели оценки остаётся у штатных инструментов локальной проверки. Подробности алгоритма и допущений приведены в [docs/architecture.md](docs/architecture.md).
...
7. В разделе **«Данные и проверки»** нажмите **«Запустить тесты»**. Результат и журнал появятся в истории; текущий набор содержит **21 тест**.

### Подтверждённые результаты

Следующие значения сохранены в репозитории для исходных данных и настроек по умолчанию:
...
| Тесты и HTTP-сценарии | **21 тест пройден**; одиночный запуск, серия и запуск тестов через HTTP — `PASS` | [web_validation.json](artifacts/web_validation.json) |
...
- `unittest` запускает 11 тестов ядра и 10 тестов локального API: решения агента, ограничения, ошибки, валидацию запросов, отмену и восстановление истории.
...
Источник — пакет участника кейса, сохранённый в [vendor/beeline_case_participants.zip](vendor/beeline_case_participants.zip). Описание полей и правил находится в [PARTICIPANT_GUIDE.md](PARTICIPANT_GUIDE.md).
...
| `data/change_tariff.csv` | История переходов; агент использует её для слабого начального ранжирования гипотез |
...
[setup_case.py](setup_case.py) проверяет SHA-256 архива и восстанавливает семь CSV. Уже существующие изменённые файлы не перезаписываются. История переходов относится к другой выборке и не рассматривается как доказательство причинного эффекта кампании.
...
- **Резервное поведение.** Если прибыльный план не подтверждён, агент может выбрать минимальную доступную кампанию с предупреждением. Положительный эффект не гарантируется. При отсутствии аудитории, контактов или допустимого перехода возвращается пустой план с диагностикой.
...
- **Бюджет времени.** Настройка 10–240 секунд в интерфейсе относится к работе агента; запуск процесса, подготовка данных и оценка требуют дополнительного времени. Фактическая длительность зависит от компьютера.
